...
from trezor.messages import StartSession
from trezor.messages import EndSession
from trezor.wire import DUMMY_CONTEXT

from apps.base import handle_StartSession, handle_EndSession
...
class TestStorageCache(unittest.TestCase):
    ...
    def test_Initialize(self):
        def call_Initialize(**kwargs):
            msg = StartSession(**kwargs)
            return await_result(handle_StartSession(DUMMY_CONTEXT, msg))

        # calling StartSession without an ID allocates a new one
        session_id = cache.start_session()
        features = call_Initialize()
        self.assertNotEqual(session_id, features.session_id)

        # calling StartSession with the current ID does not allocate a new one
