...
from trezor.enums import MessageType, InputScriptType
...
chain_list = [
    (MessageType.GetAddress, {
        ...
        "native_segwit_path": [0x80000000 + 84, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
        "nested_segwit_path": [0x80000000 + 49, 0x80000000 + 0, 0x80000000 + 0, 0, 0],
    ...
    (MessageType.SolanaGetAddress, {
        ...
        "index_pos": -2,
        "base_path": [0x80000000 + 44, 0x80000000 + 501, 0x80000000 + 0, 0x80000000 + 0],
    ...
    (MessageType.CardanoGetAddress, {
        ...
        "staking_path": [0x80000000 + 1852, 0x80000000 + 1815, 0x80000000 + 0, 2, 0],
    ...
    (MessageType.SuiGetAddress, {
        ...
        "index_pos": -1,
        "base_path": [0x80000000 + 44, 0x80000000 + 784, 0x80000000 + 0, 0x80000000 + 0, 0x80000000 + 0],
    ...
    (MessageType.PolkadotGetAddress, {
        ...
        "index_pos": -1,
        "base_path": [0x80000000 + 44, 0x80000000 + 354, 0x80000000 + 0, 0x80000000 + 0, 0x80000000 + 0],
    ...
    (MessageType.AptosGetAddress, {
        ...
        "index_pos": -1,
        "base_path": [0x80000000 + 44, 0x80000000 + 637, 0x80000000 + 0, 0x80000000 + 0, 0x80000000 + 0],
    ...
    (MessageType.AlgorandGetAddress, {
        ...
        "index_pos": -1,
        "base_path": [0x80000000 + 44, 0x80000000 + 283, 0x80000000 + 0, 0x80000000 + 0, 0x80000000 + 0],
    ...
    (MessageType.NEMGetAddress, {
        "symbol": " XEM",
        "name": "NEM",
        "msg_class": "NEMGetAddress",
        "index_pos": -1,
        "base_path": [0x80000000 + 44, 0x80000000 + 43, 0x80000000 + 0],
        "icon_96": "A:/res/chain-xem.png",
        "icon_48": "A:/res/chain-xem-48.png",
    ...
    (MessageType.StellarGetAddress, {
        "symbol": " XLM",
        "name": "Stellar",
        "msg_class": "StellarGetAddress",
        "index_pos": -1,
        "base_path": [0x80000000 + 44, 0x80000000 + 148, 0x80000000 + 0],
        "icon_96": "A:/res/chain-xlm.png",
        "icon_48": "A:/res/chain-xlm-48.png",
        "primary_color": 0xFFFFFF
    })
]
...
class AddressManager:
    ...
    def get_chain_info(self, msg_type):
    ...
    def _prepare_btc_message(self):
        ...
        return msg_class(address_n=path, show_display=False, script_type=self.btc_script_type)

    def _prepare_eth_message(self):
        """Prepare Ethereum message"""
        path = self.current_chain_info['base_path'][:]
        if self.use_ledger_path == True:
    ...
    def _prepare_ada_message(self):
        ...
        address_parameters = CardanoAddressParametersType(address_type=CardanoAddressType.BASE, address_n=path, address_n_staking=staking_path)
        ...
        msg_class = getattr(messages, self.current_chain_info['msg_class'])
        return msg_class(show_display=False, protocol_magic=764824073, network_id=1, address_parameters=address_parameters, derivation_type=derivation_type)
    ...
    def _prepare_dot_message(self):
        ...
        msg_class = getattr(messages, self.current_chain_info['msg_class'])
        return msg_class(address_n=path, show_display=False, network="polkadot", prefix=0)
    ...
    async def generate_address(self, msg_type, index=0, ctx=None):
        ...
        while state != self.STATE.FINISH and state != self.STATE.ERROR:
            if state == self.STATE.INIT:
                ...
                # address_resp = messages.Address(address="Test Address")

                ctx.primary_color = lv.color_hex(self.current_chain_info['primary_color'])
                ctx.icon_path = self.current_chain_info['icon_96']
        
                self.user_interaction = await show_address_offline(
                    ctx,
                    address=address_resp.address if msg_type != MessageType.NostrGetPublicKey else address_resp.publickey,
