...
class FullSizeWindow(lv.obj):
    ...
    def _load_anim_hor(self):
        Anim(
            480, 0, self._set_x, time=SETTINGS_MOVE_TIME, delay=SETTINGS_MOVE_DELAY
        ).start_anim()

    def _load_anim_ver(self):
        self.set_y(800)
        Anim(
            800, 0, self._set_y, time=SETTINGS_MOVE_TIME, delay=SETTINGS_MOVE_DELAY
        ).start_anim()
    ...
    def _dismiss_anim_hor(self):
        Anim(
            ...
            480,
            self._set_x,
            time=SETTINGS_MOVE_TIME,
            delay=SETTINGS_MOVE_DELAY,
