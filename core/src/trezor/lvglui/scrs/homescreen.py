...
if __debug__:
    APP_DRAWER_UP_TIME = 100
    APP_DRAWER_DOWN_TIME = 200
    APP_DRAWER_UP_DELAY = 10
...
class MainScreen(Screen):
    ...
    class AppDrawer(lv.obj):
        ...
        def set_position(self, val):
            print(f"set_position {val} ====== {lv.tick_get()}")
            self.set_y(val)
        ...
        def set_cur_path_cb_type(self, type: int):
            ...
            if type == 0:
                self.cur_up_path_cb_type.set_text(
                    f"当前主页上滑动画类型: {self.get_path_cb_str(APP_DRAWER_UP_PATH_CB)}"
                )
            elif type == 1:
                self.cur_sown_path_cb_type.set_text(
                    f"当前主页下滑动画类型: {self.get_path_cb_str(APP_DRAWER_DOWN_PATH_CB)}"
                )
            else:
...
class FingerprintSetting(Screen):
    ...
    def fresh_show(self):
        ...
        self.fingerprint_list = fingerprint.list_template()
        # assert self.fingerprint_list is not None
        if __debug__:
        ...
        if code == lv.EVENT.CLICKED:
            ...
            if target == self.add_fingerprint:
                ...
                for i, finger in enumerate(self.added_fingerprints):
                    if target == finger:
                        select_index = i
                        assert self.fingerprint_list[select_index] is not None
                        workflow.spawn(
