...
#         back_up_page_flag = await ctx.wait(ask_screen.request())
...
#                 )
#                 start_flag = await ctx.wait(start_scr.request())
#                 if start_flag == 1:
#                     # while True:
#                         print("start search .......")
#                         from trezor.lvglui.scrs.nfc import SearchDeviceScreen
#                         search_scr = SearchDeviceScreen()
#                         status_code = ctx.wait(search_scr.request())
#                         # 设备连接成功
#                         if await status_code == 2:
#                                 print("search ok")
#                                 from trezor.lvglui.scrs.nfc import TransferDataScreen
#                                 trash_scr = TransferDataScreen()
#                                 trash_scr.check_card_data()  #检查卡中是否已经存在数据
#                                 # 获取卡片状态及卡号
...
#                                 if  first_char_num == 2:   #卡片未初始化
#                                     # trash_scr.set_pin()
#                                     print("search_scr done pin 3")
#                                     # 初次设置pin码
#                                     pin = await request_lite_pin_confirm(ctx) #确认pin码流程
#                                     place_again_empty_card = True
#                                     if pin:
#                                       while place_again_empty_card:
...
#                                         start_scr_again.img.set_src("A:/res/nfc-start.png")
#                                         start_scr_again.img.align_to(
#                                         start_scr_again.subtitle, lv.ALIGN.OUT_BOTTOM_MID, 0, 52
#                                              )
#                                             #二次触卡确认
#                                         again_flag = await ctx.wait(start_scr_again.request())
#                                         #二次触卡确认
#                                         if again_flag == 1:
#                                                 #二次触卡搜索
#                                                 search_scr_agin = SearchDeviceScreen()
#                                                 status_code = await ctx.wait(search_scr_agin.request())
#                                                 # 设备二次搜索成功
#                                                 if status_code == 2:
#                                                     trash_scr_again =  TransferDataScreen()
#                                                     # 设置 pin码 和 导入助记词
#                                                     trash_scr_again.set_pin_mnemonicmphrase(pin,card_num)
#                                                     status_code = await ctx.wait(trash_scr_again.request())
#                                                     if status_code == 2:
#                                                         print("Two cards are inconsistent")
#                                                         ask_screen_tmp = FullSizeWindow(
#                                                             "Backup Completed!",# 标题：开始
#                                                             "You can recover your wallet using this card and PIN at all times.", # 内容：放置设备并点击继续
#                                                             confirm_text="Continue", # 确认按钮文本：继续
#                                                             # cancel_text=_(i18n_keys.BUTTON__NOT_NOW), # 取消按钮文本：返回
#                                                             icon_path="A:/res/success.png",
#                                                             anim_dir=0, # 动画方向
#                                                             )
#                                                         ask_screen_tmp.btn_layout_ver()  # 设置按钮垂直布局
#                                                         if await ctx.wait(ask_screen_tmp.request()):

#                                                            return  # 退出备份页面
#                                                     if status_code == 4 :  #两次卡片不符
#                                                         print("Two cards are inconsistent")
#                                                         ask_screen_tmp = FullSizeWindow(
#                                                             "Connect Failed",# 标题：开始
#                                                             "Make sure the card is close to the upper left part of the device,and then try again.", # 内容：放置设备并点击继续
#                                                             confirm_text="I got it", # 确认按钮文本：继续
#                                                             # cancel_text=_(i18n_keys.BUTTON__NOT_NOW), # 取消按钮文本：返回
...
#                                                            break
#                                         else : place_again_data_card = False
#                                     if not pin:
#                                         print("Lite pin cancelled")
#                                     print("setpin done")
#                                 if  first_char_num == 3:   #卡中已经有数据
#                                     print("pop up overlay page")
...
#                                                     trash_scr_again =  TransferDataScreen()
#                                                     # 检查 pin码 和 导入助记词
#                                                     trash_scr_again.check_pin_mnemonicmphrase(pin,card_num)
#                                                     status_code = await ctx.wait(trash_scr_again.request())
...
#                                                         ask_screen_tmp.btn_layout_ver()  # 设置按钮垂直布局
#                                                         if await ctx.wait(ask_screen_tmp.request()):
#                                                             place_again_data_card = False
#                                                     if status_code == 5 :  #reset
#                                                         print("reset device")
#                                                         ask_screen_tmp = FullSizeWindow(
#                                                             "Lite has been Reset",# 标题：开始
#                                                             "Then PIN has been entered incorrectly more than times.OneKey Lite has been self-erased to prevent brute force cracking of the backup data.", # 内容：放置设备并点击继续
#                                                             confirm_text="I got it", # 确认按钮文本：继续
#                                                             # cancel_text=_(i18n_keys.BUTTON__NOT_NOW), # 取消按钮文本：返回
#                                                             icon_path="A:/res/danger.png",
#                                                             anim_dir=0, # 动画方向
...
#                                                     if str(status_code).startswith("63C"):
#                                                         retry_count = int(str(status_code)[-1], 16)  # 提取重试次数
#                                                         print(f"PIN verification failed. Remaining retries: {retry_count}")
#                                                         ask_screen_tmp = FullSizeWindow(
#                                                                 "Lite PIN Error",  # 标题：PIN 验证失败
#                                                                 f"Data on this card will be erased after {retry_count} wrong tries.",  # 内容：显示重试次数
#                                                                 confirm_text="Try Again",  # 确认按钮文本：继续
#                                                                 cancel_text=_(i18n_keys.BUTTON__NOT_NOW), # 取消按钮文本：返回
#                                                                 icon_path="A:/res/danger.png",
#                                                                 anim_dir=0,  # 动画方向
#                                                                 )
#                                                         ask_screen_tmp.btn_layout_ver()  # 设置按钮垂直布局
#                                                         if await ctx.wait(ask_screen_tmp.request()):
#                                                              place_again_data_card = False   # 返回到放置卡片的页面
#                                             else : place_again_data_card = False
#                 else : first_placement = False
#         else: return


# async def backup_with_lite_import(
#     ctx: wire.GenericContext, mnemonics: bytes, recovery_check: bool = False
# ):
#     if not __debug__:
#             return
...
#                                 importflag = await ctx.wait(trash_scr.request())
#                                 if importflag == 2:
#                                     print("Two cards are inconsistent")
#                                     ask_screen_tmp = FullSizeWindow(
#                                                             "Wallet is Ready",# 标题：开始
#                                                             "You have successfully recovered your wallet.", # 内容：放置设备并点击继续
#                                                             confirm_text="Continue", # 确认按钮文本：继续
#                                                             # cancel_text=_(i18n_keys.BUTTON__NOT_NOW), # 取消按钮文本：返回
#                                                             icon_path="A:/res/success.png",
#                                                             anim_dir=0, # 动画方向
...
#                 else : back_up_page = False

#     return


# async def backup_with_lite_import(ctx: wire.GenericContext, mnemonics: bytes, recovery_check: bool = False):
#     if not __debug__:
#         return
...
#     def create_full_size_window(title, content, confirm_text, cancel_text=None, icon_path=None):
#         window = FullSizeWindow(
#             title, content, confirm_text=confirm_text, cancel_text=cancel_text, icon_path=icon_path, anim_dir=0
#         )
#         window.btn_layout_ver()
#         return window

#     async def show_start_screen():
#         start_scr = create_full_size_window(
#             _(i18n_keys.TITLE__GET_STARTED),
#             _(i18n_keys.CONTENT__PLACE_LITE_DEVICE_FIGURE_CLICK_CONTINUE),
...
#         start_scr.img = lv.img(start_scr.content_area)
#         start_scr.img.set_src("A:/res/nfc-start.png")
#         start_scr.img.align_to(start_scr.subtitle, lv.ALIGN.OUT_BOTTOM_MID, 0, 52)
#         return await ctx.wait(start_scr.request())
...
#         trash_scr = TransferDataScreen()
#         trash_scr.import_pin_mnemonicmphrase(pin)
#         import_flag = await ctx.wait(trash_scr.request())
#         if import_flag == 2:
#             success_screen = create_full_size_window(
#                 "Wallet is Ready",
#                 "You have successfully recovered your wallet.",
#                 "Continue",
#                 icon_path="A:/res/success.png"
#             )
#             if await ctx.wait(success_screen.request()):
#                 return True
#         return False
...
#                 if status_code == 2:
#                     print("search ok")
#                     if await handle_wallet_recovery(pin):
#                         return
#         else:
#             back_up_page = False
