...
from ur.fountain_utils import choose_degree, choose_fragments, shuffled
...
class TestUR(BaseClass):
    ...
    def test_bytewords_1(self):
        ...
        assert (
            Bytewords.encode(Bytewords_Style_standard, input)
            == "able acid also lava zoom jade need echo taxi"
        )
        assert (
            Bytewords.encode(Bytewords_Style_uri, input)
            == "able-acid-also-lava-zoom-jade-need-echo-taxi"
        )
        ...
        assert (
            Bytewords.decode(
                Bytewords_Style_standard, "able acid also lava zoom jade need echo taxi"
            )
            == input
        )
        assert (
            Bytewords.decode(
                Bytewords_Style_uri, "able-acid-also-lava-zoom-jade-need-echo-taxi"
            )
            == input
        )
        ...
        self.assertRaises(
            ValueError,
            lambda: Bytewords.decode(
                Bytewords_Style_standard, "able acid also lava zoom jade need echo wolf"
            ),
        )
        ...
        self.assertRaises(
            ValueError,
            lambda: Bytewords.decode(
                Bytewords_Style_uri, "able-acid-also-lava-zoom-jade-need-echo-wolf"
            ),
        )

        self.assertRaises(
            ValueError,
            lambda: Bytewords.decode(Bytewords_Style_minimal, "aeadaolazmjendeowf"),
        )

        # too short
        self.assertRaises(
            ValueError, lambda: Bytewords.decode(Bytewords_Style_standard, "wolf")
        )

        self.assertRaises(
            ValueError, lambda: Bytewords.decode(Bytewords_Style_standard, "")
        )
    ...
    def test_partition_and_join(self):
        ...
        fragment_len = FountainEncoder.find_nominal_fragment_length(
            len(message), 10, 100
        )
