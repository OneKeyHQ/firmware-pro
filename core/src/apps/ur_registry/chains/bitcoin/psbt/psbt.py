...
from .tx import COutPoint, CTransaction, CTxIn, CTxInWitness, CTxOut
...
def deserialize_HDKeypath(
    f: Readable,
    key: bytes,
    hd_keypaths: Dict[bytes, KeyOriginInfo],
    ...
    :param hd_keypaths: Dictionary of public key bytes to their.
    :param expected_sizes: List of key lengths expected for the keypair being deserialized.
    """
    if len(key) not in expected_sizes:
        raise Exception(
            f"Size of key was not the expected size for the type partial signature pubkey. Length: {len(key)}"
        )
    pubkey = key[1:]
...
def serialize_HDKeypath(hd_keypaths: Dict[bytes, KeyOriginInfo], type: bytes) -> bytes:
    """
    :meta private:
...
class PartiallySignedInput:
    ...
    def __init__(self, version: int) -> None:
        self.non_witness_utxo: CTransaction | None = None
        self.witness_utxo: CTxOut | None = None
        self.partial_sigs: Dict[bytes, bytes] = {}
        self.sighash: int | None = None
        ...
        self.prev_txid = b""
        self.prev_out: int | None = None
        self.sequence: int | None = None
        self.time_locktime: int | None = None
        self.height_locktime: int | None = None
        self.tap_key_sig = b""
    ...
    def deserialize(self, f: Readable) -> None:
        ...
        while True:
            ...
            key_type = deser_compact_size(BufferReader(key))
            ...
            if key_type == PartiallySignedInput.PSBT_IN_NON_WITNESS_UTXO:
                if key in key_lookup:
                    ...
                    raise Exception("non witness utxo key is more than one byte type")
                self.non_witness_utxo = CTransaction()
                utxo_bytes = BufferReader(deser_string(f))
                ...
                if key in key_lookup:
                    ...
                    raise Exception("witness utxo key is more than one byte type")
                self.witness_utxo = CTxOut()
                tx_out_bytes = BufferReader(deser_string(f))
                ...
                if key in key_lookup:
                    ...
                    raise Exception("sighash key is more than one byte type")
                ...
                if key in key_lookup:
                    ...
                    raise Exception("redeemScript key is more than one byte type")
                ...
                if key in key_lookup:
                    ...
                    raise Exception("witnessScript key is more than one byte type")
                self.witness_script = deser_string(f)
            elif key_type == PartiallySignedInput.PSBT_IN_BIP32_DERIVATION:
                deserialize_HDKeypath(f, key, self.hd_keypaths, [34, 66])
                ...
                if key in key_lookup:
                    ...
                    raise Exception("final scriptSig key is more than one byte type")
                ...
                witness_bytes = BufferReader(deser_string(f))
                self.final_script_witness.deserialize(witness_bytes)
            elif key_type == PartiallySignedInput.PSBT_IN_PREVIOUS_TXID:
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, input previous txid is already provided"
                    )
                elif len(key) != 1:
                    raise Exception("Previous txid key is more than one byte type")
                txid = deser_string(f)
                if len(txid) != 32:
                    raise Exception("Previous txid is not 32 bytes")
                self.prev_txid = txid
            elif key_type == PartiallySignedInput.PSBT_IN_OUTPUT_INDEX:
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, input previous output index is already provided"
                    )
                elif len(key) != 1:
                    raise Exception(
                        "Previous output index key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Previous output index is not 4 bytes")
                self.prev_out = struct.unpack("<I", v)[0]
            elif key_type == PartiallySignedInput.PSBT_IN_SEQUENCE:
                pass
                if key in key_lookup:
                    raise Exception("Duplicate key, input sequence is already provided")
                elif len(key) != 1:
                    raise Exception("Input sequence key is more than one byte type")
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Input sequence is not 4 bytes")
                self.sequence = struct.unpack("<I", v)[0]
            elif key_type == PartiallySignedInput.PSBT_IN_REQUIRED_TIME_LOCKTIME:
                pass
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, input required time based locktime is already provided"
                    )
                elif len(key) != 1:
                    raise Exception(
                        "Input time based locktime key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Input time based locktime is not 4 bytes")
                self.time_locktime = struct.unpack("<I", v)[0]
            elif key_type == PartiallySignedInput.PSBT_IN_REQUIRED_HEIGHT_LOCKTIME:
                pass
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, input required height based locktime index is already provided"
                    )
                elif len(key) != 1:
                    raise Exception(
                        "Input height based locktime key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Input height based locktime is not 4 bytes")
                ...
                if key in key_lookup:
                    ...
                    raise Exception(
                        "Input Taproot leaf script key's control block is not valid"
                    )
                script = deser_string(f)
                if len(script) == 0:
                    raise Exception("Input Taproot leaf script cannot be empty")
                ...
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, input Taproot BIP 32 keypath already provided"
                    )
                elif len(key) != 33:
                    raise Exception("Input Taproot BIP 32 keypath key is not 33 bytes")
                xonly = key[1:33]
                value = deser_string(f)
                vs = BufferReader(value)
                num_hashes = deser_compact_size(vs)
                leaf_hashes = set()
                for _ in range(0, num_hashes):
                    leaf_hashes.add(vs.read(32))
                self.tap_bip32_paths[xonly] = (
                    leaf_hashes,
                    KeyOriginInfo.deserialize(vs.read()),
                )
                ...
                if len(self.tap_internal_key) != 32:
                    raise Exception("Input Taproot internal key is not 32 bytes")
                ...
                if len(self.tap_merkle_root) != 32:
                    raise Exception("Input Taproot merkle root is not 32 bytes")
            else:
    ...
    def serialize(self) -> bytes:
        ...
        if self.non_witness_utxo:
            r += ser_string(
                ser_compact_size(PartiallySignedInput.PSBT_IN_NON_WITNESS_UTXO)
            )
        ...
        if len(self.final_script_sig) == 0 and self.final_script_witness.is_null():
            for pubkey, sig in sorted(self.partial_sigs.items()):
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_PARTIAL_SIG) + pubkey
                )
                r += ser_string(sig)

            if self.sighash is not None:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_SIGHASH_TYPE)
                )
                r += ser_string(struct.pack("<I", self.sighash))

            if len(self.redeem_script) != 0:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_REDEEM_SCRIPT)
                )
                r += ser_string(self.redeem_script)

            if len(self.witness_script) != 0:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_WITNESS_SCRIPT)
                )
                r += ser_string(self.witness_script)

            r += serialize_HDKeypath(
                self.hd_keypaths,
                ser_compact_size(PartiallySignedInput.PSBT_IN_BIP32_DERIVATION),
            )

            if len(self.tap_key_sig) != 0:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_TAP_KEY_SIG)
                )
                r += ser_string(self.tap_key_sig)

            for (xonly, leaf_hash), sig in self.tap_script_sigs.items():
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_TAP_SCRIPT_SIG)
                    + xonly
                    + leaf_hash
                )
                r += ser_string(sig)
            ...
            for (script, leaf_ver), control_blocks in self.tap_scripts.items():
                for control_block in control_blocks:
                    r += ser_string(
                        ser_compact_size(PartiallySignedInput.PSBT_IN_TAP_LEAF_SCRIPT)
                        + control_block
                    )
            ...
            for xonly, (leaf_hashes, origin) in self.tap_bip32_paths.items():
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_TAP_BIP32_DERIVATION)
                    + xonly
                )
            ...
            if len(self.tap_internal_key) != 0:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_TAP_INTERNAL_KEY)
                )
                r += ser_string(self.tap_internal_key)

            if len(self.tap_merkle_root) != 0:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_TAP_MERKLE_ROOT)
                )
                r += ser_string(self.tap_merkle_root)

        if len(self.final_script_sig) != 0:
            r += ser_string(
                ser_compact_size(PartiallySignedInput.PSBT_IN_FINAL_SCRIPTSIG)
            )
            r += ser_string(self.final_script_sig)

        if not self.final_script_witness.is_null():
            r += ser_string(
                ser_compact_size(PartiallySignedInput.PSBT_IN_FINAL_SCRIPTWITNESS)
            )
        ...
        if self.version >= 2:
            if len(self.prev_txid) != 0:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_PREVIOUS_TXID)
                )
                r += ser_string(self.prev_txid)

            if self.prev_out is not None:
                r += ser_string(
                    ser_compact_size(PartiallySignedInput.PSBT_IN_OUTPUT_INDEX)
                )
            ...
            if self.time_locktime is not None:
                r += ser_string(
                    ser_compact_size(
                        PartiallySignedInput.PSBT_IN_REQUIRED_TIME_LOCKTIME
                    )
                )
            ...
            if self.height_locktime is not None:
                r += ser_string(
                    ser_compact_size(
                        PartiallySignedInput.PSBT_IN_REQUIRED_HEIGHT_LOCKTIME
                    )
                )
...
class PartiallySignedOutput:
    ...
    def deserialize(self, f: Readable) -> None:
        ...
        while True:
            ...
            if key_type == PartiallySignedOutput.PSBT_OUT_REDEEM_SCRIPT:
                ...
                if key in key_lookup:
                    raise Exception("Duplicate key, output amount already provided")
                elif len(key) != 1:
                    raise Exception("Output amount key is more than one byte type")
                v = deser_string(f)
                if len(v) != 8:
                    raise Exception("Output amount is not 8 bytes")
                self.amount = struct.unpack("<q", v)[0]
            elif key_type == PartiallySignedOutput.PSBT_OUT_SCRIPT:
                if key in key_lookup:
                    raise Exception("Duplicate key, output script already provided")
                elif len(key) != 1:
                    raise Exception("Output script key is more than one byte type")
                ...
                self.tap_internal_key = deser_string(f)
                if len(self.tap_internal_key) != 32:
                    raise Exception("Output Taproot internal key is not 32 bytes")
                ...
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, output Taproot BIP 32 keypath already provided"
                    )
                elif len(key) != 33:
                    raise Exception("Output Taproot BIP 32 keypath key is not 33 bytes")
                xonly = key[1:33]
                value = deser_string(f)
                vs = BufferReader(value)
                num_hashes = deser_compact_size(vs)
        ...
        if self.version >= 2:
            ...
            if len(self.script) == 0:
                raise Exception("PSBT_OUTPUT_SCRIPT is required in PSBTv2")
    ...
    def serialize(self) -> bytes:
        ...
        if len(self.redeem_script) != 0:
            r += ser_string(
                ser_compact_size(PartiallySignedOutput.PSBT_OUT_REDEEM_SCRIPT)
            )
            r += ser_string(self.redeem_script)

        if len(self.witness_script) != 0:
            r += ser_string(
                ser_compact_size(PartiallySignedOutput.PSBT_OUT_WITNESS_SCRIPT)
            )
            r += ser_string(self.witness_script)

        r += serialize_HDKeypath(
            self.hd_keypaths,
            ser_compact_size(PartiallySignedOutput.PSBT_OUT_BIP32_DERIVATION),
        )
        ...
        if len(self.tap_internal_key) != 0:
            r += ser_string(
                ser_compact_size(PartiallySignedOutput.PSBT_OUT_TAP_INTERNAL_KEY)
            )
            r += ser_string(self.tap_internal_key)
        ...
        for xonly, (leaf_hashes, origin) in self.tap_bip32_paths.items():
            r += ser_string(
                ser_compact_size(PartiallySignedOutput.PSBT_OUT_TAP_BIP32_DERIVATION)
                + xonly
            )
            value = ser_compact_size(len(leaf_hashes))
            for lh in leaf_hashes:
...
class PSBT:
    ...
    def __init__(self, tx: CTransaction | None = None) -> None:
        ...
        self.tx_version: int | None = None
        self.fallback_locktime: int | None = None
        self.tx_modifiable: int | None = None
    ...
    def deserialize(self, psbt: bytes) -> None:
        ...
        while True:
            ...
            if key_type == PSBT.PSBT_GLOBAL_UNSIGNED_TX:
                ...
                if key in key_lookup:
                    raise Exception("Duplicate key, unsigned tx already provided")
                elif len(key) > 1:
                    raise Exception("Global unsigned tx key is more than one byte type")

                # read in value
                tx_bytes = BufferReader(deser_string(f))
                ...
                deserialize_HDKeypath(f, key, self.xpub, [79])
            elif key_type == PSBT.PSBT_GLOBAL_TX_VERSION:
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, global transaction version is already provided"
                    )
                elif len(key) > 1:
                    raise Exception(
                        "Global transaction version key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Global transaction version is not 4 bytes")
                self.tx_version = struct.unpack("<I", v)[0]
            elif key_type == PSBT.PSBT_GLOBAL_FALLBACK_LOCKTIME:
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, global fallback locktime is already provided"
                    )
                elif len(key) > 1:
                    raise Exception(
                        "Global fallback locktime key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Global fallback locktime is not 4 bytes")
                self.fallback_locktime = struct.unpack("<I", v)[0]
            elif key_type == PSBT.PSBT_GLOBAL_INPUT_COUNT:
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, global input count is already provided"
                    )
                elif len(key) > 1:
                    raise Exception("Global input count key is more than one byte type")
                ...
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, global output count is already provided"
                    )
                elif len(key) > 1:
                    raise Exception(
                        "Global output count key is more than one byte type"
                    )
                _ = deser_compact_size(f)  # Value length, we can ignore this
                ...
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, global tx modifiable flags is already provided"
                    )
                elif len(key) > 1:
                    raise Exception(
                        "Global tx modifiable flags key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 1:
                    raise Exception("Global tx modifiable flags is not 1 bytes")
                self.tx_modifiable = struct.unpack("<B", v)[0]
            elif key_type == PSBT.PSBT_GLOBAL_VERSION:
                if key in key_lookup:
                    raise Exception(
                        "Duplicate key, global PSBT version is already provided"
                    )
                elif len(key) > 1:
                    raise Exception(
                        "Global PSBT version key is more than one byte type"
                    )
                v = deser_string(f)
                if len(v) != 4:
                    raise Exception("Global PSBT version is not 4 bytes")
        ...
        if self.version == 0:
            ...
            if self.tx.is_null():
                raise Exception("No unsigned transaction was provided")
            # Make sure no v2 fields are present
            if self.tx_version is not None:
                raise Exception("PSBT_GLOBAL_TX_VERSION is not allowed in PSBTv0")
            if self.fallback_locktime is not None:
                raise Exception(
                    "PSBT_GLOBAL_FALLBACK_LOCKTIME is not allowed in PSBTv0"
                )
            if input_count is not None:
                raise Exception("PSBT_GLOBAL_INPUT_COUNT is not allowed in PSBTv0")
            if output_count is not None:
                raise Exception("PSBT_GLOBAL_OUTPUT_COUNT is not allowed in PSBTv0")
            if self.tx_modifiable is not None:
                raise Exception("PSBT_GLOBAL_TX_MODIFIABLE is not allowed in PSBTv0")

        # Disallow v1
        if self.version == 1:
            raise Exception("There is no PSBT version 1")
        if self.version >= 2:
            ...
            if input_count is None:
                raise Exception("PSBT_GLOBAL_INPUT_COUNT is required in PSBTv2")
            if output_count is None:
                raise Exception("PSBT_GLOBAL_OUTPUT_COUNT is required in PSBTv2")
            # Unsigned tx is disallowed
            if not self.tx.is_null():
                raise Exception("PSBT_GLOBAL_UNSIGNED_TX is not allowed in PSBTv2")
        ...
        for i in range(input_count):
            ...
            if psbt_in.non_witness_utxo:
                ...
                if psbt_in.non_witness_utxo.hash != prev_txid:
                    raise Exception("Non-witness UTXO does not match outpoint hash")
