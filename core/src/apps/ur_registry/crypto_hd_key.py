...
from .crypto_coin_info import *
from .crypto_key_path import *
from .ur_py.ur.cbor_lite import CBORDecoder, CBOREncoder
from .ur_py.ur.ur import UR
from .ur_py.ur.ur_decoder import URDecoder
...
class CryptoHDKey:
    ...
    def get_parent_fingerprint(self):
    ...
    def get_bip32_key(self):
        ...
        if self.get_is_master() is True:
            ...
            if self.get_origin() is not None:
                depth = len(self.get_origin().get_components())
                index = self.get_origin().get_components()[-1].get_canonical_index()
    ...
    def decode(decoder):
        ...
        for _ in range(size):
            ...
            if key == IS_MASTER:
                ...
                raise Exception("Expected key {}".format(key))
        return key_path
