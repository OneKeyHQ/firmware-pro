    ...
    def new(index, hardened):
        ...
        if index is not None:
            if index & PathComponent.HARDEN_BIT != 0:
                raise f"Invalid index {index} - most significant bit cannot be set"
            p.index = index
            p.wildcard = False
            p.hardened = hardened
        else:
...
class CryptoKeyPath:
    ...
    def get_path(self):
        ...
        for component in self.components:
            if component.wildcard == True and component.hardened == True:
                path += "*'"
            if component.wildcard == True and component.hardened == False:
                path += "*"
            if component.wildcard == False and component.hardened == True:
                path += f"{component.index}'"
            if component.wildcard == False and component.hardened == False:
                path += f"{component.index}"
            path += "/"
        return path[:-1]
