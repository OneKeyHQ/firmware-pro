...
class Account:
    ...
    def from_crypto_hd_key(hdkey: CryptoHDKey):
        coin_type = hdkey.get_origin().get_components()[1].get_index()
        chain = get_coin_name(coin_type)
        path = hdkey.get_origin().get_path()
        ...
        source_fingerprint = (
            hdkey.get_origin().get_source_fingerprint()
            if hdkey.get_origin() is not None
            else None
        )
        xfp = (
        ...
        return Account(
            ...
            "m/{}".format(path) if path is not None else path,
