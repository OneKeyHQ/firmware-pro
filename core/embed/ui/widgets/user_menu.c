...
{
    CreateGeneralNavigationBar(parent, NULL);
    lv_obj_t *bg = lv_obj_create(parent);
    lv_obj_set_style_bg_color(bg, lv_color_make(0x20, 0x20, 0x20), 0);
    lv_obj_set_style_border_width(bg, 0, 0);
    lv_obj_set_style_radius(bg, 0, 0);
    lv_obj_set_style_pad_all(bg, 0, 0);
    lv_obj_set_size(bg, MY_DISP_HOR_RES - 40, MY_DISP_VER_RES - STATUS_BAR_HEIGHT - NAVIGATION_BAR_HEIGHT - 40);
    lv_obj_align(bg, LV_ALIGN_TOP_MID, 0, NAVIGATION_BAR_HEIGHT + 20);
    lv_obj_set_style_clip_corner(bg, true, 0);
    lv_obj_set_scrollbar_mode(bg, LV_SCROLLBAR_MODE_ACTIVE);
    lv_obj_clear_flag(bg, LV_OBJ_FLAG_SCROLL_ELASTIC);
    lv_obj_clear_flag(bg, LV_OBJ_FLAG_SCROLL_MOMENTUM);
    ...
    for (uint32_t i = 0; i < itemCount; i++) {
        ...
        lv_obj_set_size(btn, MY_DISP_HOR_RES - 40, MENU_ITEM_HEIGHT - 6);
