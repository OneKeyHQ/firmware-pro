...
#if (!defined(CLIB_NO_FP_INIT) && !defined(__ARM_ARCH_7A__) && \
...
static uint32_t os_libspace[OS_THREAD_LIBSPACE_NUM + 1]
                           [LIBSPACE_SIZE / sizeof(uint32_t)];
...
/* Define the number of Mutexes used by standard C/C++ library for stream
 * protection */
...
int _mutex_initialize(mutex *m) {
  ...
  *m = NULL;
  ...
  /* FreeRTOS disables interrupts when its API is called before the kernel is
   * started. */
  /* This is pre-main context and since interrupts shall not happen before
   * reaching    */
  /* main we can re-enable interrupts and have consistent state when main gets
   * called. */
...
void _mutex_acquire(mutex *m) {
  /* Don't allow mutex operations when the kernel is not switching tasks and
   * also   */
  /* block protection when in interrupt. Using stdio streams in interrupt is bad
   */
  /* practice, but some applications call printf as last resort for debug
   * purposes. */
