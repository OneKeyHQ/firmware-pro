...
static HAL_StatusTypeDef dma2d_init(DMA2D_HandleTypeDef* hdma2d)
{
    if ( hdma2d->Instance != DMA2D )
        return HAL_ERROR; // Check instance
...
static HAL_StatusTypeDef ltdc_init(LTDC_HandleTypeDef* hltdc)
{
    ...
    hltdc->Init.AccumulatedActiveW = lcd_params.hsync + lcd_params.hres + lcd_params.hbp -
...
ltdc_layer_config(LTDC_HandleTypeDef* hltdc, uint32_t layer_index, LTDC_LAYERCONFIG* config)
{
...
static HAL_StatusTypeDef dsi_host_init(DSI_HandleTypeDef* hdsi)
{
    ...
    // Configure DSI clock source
    {
        ...
        HAL_StatusTypeDef result =
            HAL_RCCEx_PeriphCLKConfig(&PeriphClkInitStruct); // Configure peripheral clock
        if ( result != HAL_OK )
        {
            return result;
        }
    ...
    // Configure DSI video mode parameters
    {
        ...
        VidCfg.HorizontalLine =
            ((lcd_params.hres + lcd_params.hsync + lcd_params.hbp + lcd_params.hfp) * DSI_FREQ) /
...
void fb_fill_rect(uint32_t x_pos, uint32_t y_pos, uint32_t width, uint32_t height, uint32_t color)
{
    ...
    /* Fill rectangle */
    fb_fill_buffer((uint32_t*)address, width, height, (lcd_params.hres - width), color);
}
...
void dma2d_copy_buffer(uint32_t* pSrc, uint32_t* pDst, uint16_t x, uint16_t y, uint16_t xsize, uint16_t ysize)
{
    ...
    hlcd_dma2d.Init.AlphaInverted = DMA2D_REGULAR_ALPHA; /* Do not invert output Alpha */
    hlcd_dma2d.Init.RedBlueSwap = DMA2D_RB_REGULAR;      /* Do not swap output red-blue */
    ...
    hlcd_dma2d.LayerCfg[1].RedBlueSwap = DMA2D_RB_REGULAR;      /* Do not swap foreground red-blue */
    hlcd_dma2d.LayerCfg[1].AlphaInverted = DMA2D_REGULAR_ALPHA; /* Do not invert foreground Alpha */
...
void dma2d_copy_ycbcr_to_rgb(
    uint32_t* pSrc, uint32_t* pDst, uint16_t copy_width, uint16_t copy_height, uint32_t ChromaSampling,
    uint16_t output_line_width, uint16_t input_line_width
)
{
    ...
    dma2d_result = HAL_DMA2D_Start(&hlcd_dma2d, (uint32_t)pSrc, (uint32_t)pDst, copy_width, copy_height);
    if ( dma2d_result != HAL_OK )
    {
        return;
    }
...
{ // Initialize CoverBackground content
    ...
    { // If current display address is valid
        // Use DMA2D to copy entire Layer1 content to Layer2 for background
        // consistency
        dma2d_copy_buffer(
            (uint32_t*)g_current_display_addr, (uint32_t*)LAYER2_MEMORY_BASE, 0, 0, lcd_params.hres,
            lcd_params.vres
        ); // DMA2D copy
    }
    else
    {
        ...
        dma2d_copy_buffer(
            (uint32_t*)FMC_SDRAM_LTDC_BUFFER_ADDRESS, (uint32_t*)LAYER2_MEMORY_BASE, 0, 0, lcd_params.hres,
            lcd_params.vres
        ); // DMA2D copy
    }
...
__attribute__((used)) void lcd_cover_background_show(void)
{
    ...
    SCB_CleanDCache_by_Addr(
        (uint32_t*)LAYER2_MEMORY_BASE, lcd_params.hres * lcd_params.vres * lcd_params.bbp
    );
    ...
    while ( timeout-- > 0 )
    {
        if ( hlcd_ltdc.Instance->CDSR & 0x01 )
        { // Check VSync status
            break;
        }
    }
...
__attribute__((used)) void lcd_cover_background_load_jpeg(const char* jpeg_path)
{
    ...
    uint32_t dest_address = (uint32_t)(layer2_buffer + top_padding * lcd_params.hres);
    dma2d_copy_ycbcr_to_rgb(
        (uint32_t*)jpeg_output_address, (uint32_t*)dest_address, adjusted_width, height, subsampling,
        lcd_params.hres,
        width
    ); // Pass screen width for proper line stride,
