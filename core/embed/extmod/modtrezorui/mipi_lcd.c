...
#define LTDC_FREQ \
    (uint32_t)(HSE_VALUE / 5 * (lcd_params.ltdc_pll.PLL3N + (lcd_params.ltdc_pll.PLL3FRACN / 8192.0F)) / lcd_params.ltdc_pll.PLL3R / 1000)
...
// + LCD_VFP + LCD_VSW))
#define FPS_TARGET                                                                       \
    (uint32_t)((float)LTDC_FREQ                                                          \
               / ((lcd_params.hres + lcd_params.hbp + lcd_params.hfp + lcd_params.hsync) \
...
static HAL_StatusTypeDef dsi_host_init(DSI_HandleTypeDef* hdsi)
{
    ...
    // video mode
    {
        ...
        VidCfg.HorizontalLine = ((lcd_params.hres + lcd_params.hsync + lcd_params.hbp + lcd_params.hfp) * DSI_FREQ) / LTDC_FREQ;
