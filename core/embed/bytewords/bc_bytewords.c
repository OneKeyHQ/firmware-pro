...
#define htonl_custom(num) \
    ((((uint32_t)(num) & 0xFF000000U) >> 24) | \
...
static uint32_t crc32n(const uint8_t* bytes, size_t len) {
    return htonl_custom(crc32(bytes, len));
}

static bool decode_word(const char* in_word, size_t word_len, uint8_t* out_index) {
    
    const size_t dim = 26;
    static int16_t array[26 * 26];
