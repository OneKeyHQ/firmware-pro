...
#define ExecuteCheck_HAL_OK(func_call) ExecuteCheck_ADV(func_call, HAL_OK, { return false; })
...
static bool motor_timer_init()
{
    ...
    ExecuteCheck_HAL_OK(HAL_TIMEx_MasterConfigSynchronization(&TIM7_Handle, &sMasterConfig));
...
void motor_timer_play(MOTOR_ACTION* act_list, size_t act_list_len)
{
    ...
    _act_list_index = &act_list[0]; // explicitly point to first element in the array
...
void motor_resonant_finder(uint16_t dur_f, uint16_t dur_r, uint16_t dur_b)
{
    ...
    motor_cpu_play(MAL_single_shot, sizeof(MAL_single_shot) / sizeof(MOTOR_ACTION));
}