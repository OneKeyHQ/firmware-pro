...
int jpeg_decode_file_size(uint32_t* size)
{
    if ( file_operation.mode == JPEG_FILE_LVGL )
    {
        ...
        if ( lv_fs_seek(&file_operation.lvgl_file, 0, LV_FS_SEEK_END) != LV_FS_RES_OK )
        {
            return -1;
        }
...
{
    return Jpeg_HWDecodingEnd;
}
int jpeg_get_decode_error(void)
{
    return Jpeg_HWDecodingError;
}
...
void HAL_JPEG_MspInit(JPEG_HandleTypeDef* hjpeg)
{
    ...
    hmdmaIn.Init.Request = MDMA_REQUEST_JPEG_INFIFO_TH; /* Set the MDMA HW trigger to JPEG Input FIFO
                                                           Threshold flag*/
    hmdmaIn.Init.TransferTriggerMode = MDMA_BUFFER_TRANSFER;
    hmdmaIn.Init.BufferTransferLength = 32;             /*Set the MDMA buffer size to the JPEG FIFO threshold size i.e 32
                                                           bytes (8 words)*/
    ...
    hmdmaOut.Init.Request = MDMA_REQUEST_JPEG_OUTFIFO_TH; /* Set the MDMA HW trigger to JPEG Output
                                                             FIFO Threshold flag*/
    hmdmaOut.Init.TransferTriggerMode = MDMA_BUFFER_TRANSFER;
    hmdmaOut.Init.BufferTransferLength = 32;              /*Set the MDMA buffer size to the JPEG FIFO threshold size i.e 32
                                                             bytes (8 words)*/
...
void HAL_JPEG_GetDataCallback(JPEG_HandleTypeDef* hjpeg, uint32_t NbDecodedData)
{
    ...
    if ( g_inputJpegOffset < g_inputJpegSize )
    {
        ...
        HAL_JPEG_ConfigInputBuffer(hjpeg, g_inputJpegBuffer + g_inputJpegOffset, indata_len);
    }
}
...
void HAL_JPEG_DataReadyCallback(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataOut, uint32_t OutDataLength)
{
    ...
    if ( g_outputBufferOffset + CHUNK_SIZE_OUT > FMC_SDRAM_JPEG_OUTPUT_DATA_BUFFER_LEN )
    {
...
int jpeg_decode_start(const char* path)
{
    ...
    if ( jpeg_decode_file_read(g_inputJpegBuffer, file_size, &g_inputJpegSize) == 0 )
    {
        enable_irq(state);
    }
    else
    ...
    HAL_JPEG_Decode_DMA(&JPEG_Handle, g_inputJpegBuffer, indata_len, g_outputBuffer, CHUNK_SIZE_OUT);
...
int jped_decode(char* path, uint32_t address)
{
    ...
    if ( jpeg_decode_start(path) == 0 )
    {
        ...
        dma2d_copy_ycbcr_to_rgb(
            (uint32_t*)address, (uint32_t*)lcd_get_src_addr(), JPEG_Info.ImageWidth, JPEG_Info.ImageHeight, JPEG_Info.ChromaSubsampling
        );
        return 0;
    }
    return -1;
